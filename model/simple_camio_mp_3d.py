...
class InteractionPolicyOBJObject:
    ...
    def project_vertices(self, R, T):
        vertices, _ = cv.projectPoints(
            self.vertices_3d, R, T, self.intrinsic_matrix, None
        )
    ...
    def draw_points(self, img):
        is_visible = np.linalg.norm(self.mid_point - self.T) > np.linalg.norm(
            self.vertices_3d - self.T, axis=1
        )
        for pt in self.vertices[is_visible, :]:
            cv.circle(
                img, (int(pt[0]), int(pt[1])), self.D_THRESHOLD, [255, 255, 255], 2
            )
        return img
    ...
    def push_gesture(self, position):
        ...
        idx = np.linspace(
            0, len(is_visible), len(is_visible), endpoint=False, dtype=int
        )[is_visible]
        min_idx, dist = find_closest_point(position, self.vertices[is_visible, :])
        self.zone_filter[self.zone_filter_cnt] = self.map_obj.vertex_reg_id[
            idx[min_idx]
        ]
